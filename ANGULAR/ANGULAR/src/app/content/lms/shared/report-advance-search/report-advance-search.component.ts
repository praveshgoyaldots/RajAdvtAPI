...
export class ReportAdvanceSearchComponent {
  ...
  resetAdvanceSearch() {
    ...
    this.model.EntryDateFrom = AppSetting.DefaultStartDate;
    this.model.EntryDateTo = AppSetting.DefaultEndDate;
    this.model.LetterType = this.model.haveAttachment = '';
  }

  getDropdownList() {
    this._commonService.GetAllDDL(AppSetting.LMS_DDLKeyForReportAdvanceSearch).subscribe(
      data => {
        if (data.IsSuccess) {
