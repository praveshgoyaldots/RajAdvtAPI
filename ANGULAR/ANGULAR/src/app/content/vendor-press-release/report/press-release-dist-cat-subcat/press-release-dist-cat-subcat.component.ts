...
import { DistCatSubcatModel } from "src/app/Shared/Model/TenderPressRelease/Report/press-release-summary-report-model";
...
@Component({
  ...
  templateUrl: "./press-release-dist-cat-subcat.component.html",
  styleUrls: ["./press-release-dist-cat-subcat.component.css"],
})
export class PressReleaseDistCatSubcatComponent implements OnInit {
  indexModel: PressReleaseFilterModel;
  summaryReportList: DistCatSubcatModel[];
  dataSource: MatTableDataSource<DistCatSubcatModel>;
  totalEntries: number;
  totalRecords: number;
  ...
  GetList() {
    this._pressReleaseSummaryReportService
      ...
      .subscribe(
        (data) => {
          if (data.IsSuccess) {
            this.summaryReportList = <DistCatSubcatModel[]>data.Data.Data;
            this.dataSource = new MatTableDataSource<DistCatSubcatModel>(
