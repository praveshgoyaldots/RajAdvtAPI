...
import { DepartmentDistrictModel } from "src/app/Shared/Model/TenderPressRelease/Report/press-release-summary-report-model";
...
@Component({
  ...
  templateUrl: "./press-release-department-district.component.html",
  styleUrls: ["./press-release-department-district.component.css"],
})
export class PressReleaseDepartmentDistrictComponent implements OnInit {
  indexModel: PressReleaseFilterModel;
  summaryReportList: DepartmentDistrictModel[];
  dataSource: MatTableDataSource<DepartmentDistrictModel>;
  totalEntries: number;
  totalRecords: number;
  ...
  GetList() {
    this._pressReleaseSummaryReportService
      ...
      .subscribe(
        (data) => {
          if (data.IsSuccess) {
            this.summaryReportList = <DepartmentDistrictModel[]>data.Data.Data;
            this.dataSource = new MatTableDataSource<DepartmentDistrictModel>(
