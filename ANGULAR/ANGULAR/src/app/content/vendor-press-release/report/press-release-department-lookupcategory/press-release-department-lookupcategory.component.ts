...
import { DeptLookupCatModel } from "src/app/Shared/Model/TenderPressRelease/Report/press-release-summary-report-model";
...
@Component({
  ...
  templateUrl: "./press-release-department-lookupcategory.component.html",
  styleUrls: ["./press-release-department-lookupcategory.component.css"],
})
export class PressReleaseDepartmentLookupcategoryComponent implements OnInit {
  indexModel: PressReleaseFilterModel;
  summaryReportList: DeptLookupCatModel[];
  dataSource: MatTableDataSource<DeptLookupCatModel>;
  totalEntries: number;
  totalRecords: number;
  ...
  GetList() {
    this._pressReleaseSummaryReportService
      ...
      .subscribe(
        (data) => {
          if (data.IsSuccess) {
            this.summaryReportList = <DeptLookupCatModel[]>data.Data.Data;
            this.dataSource = new MatTableDataSource<DeptLookupCatModel>(
