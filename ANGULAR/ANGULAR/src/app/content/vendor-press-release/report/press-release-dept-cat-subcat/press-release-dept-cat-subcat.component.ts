...
import { DeptCatSubcatModel } from "src/app/Shared/Model/TenderPressRelease/Report/press-release-summary-report-model";
...
@Component({
  ...
  templateUrl: "./press-release-dept-cat-subcat.component.html",
  styleUrls: ["./press-release-dept-cat-subcat.component.css"],
})
export class PressReleaseDeptCatSubcatComponent implements OnInit {
  indexModel: PressReleaseFilterModel;
  summaryReportList: DeptCatSubcatModel[];
  dataSource: MatTableDataSource<DeptCatSubcatModel>;
  totalEntries: number;
  totalRecords: number;
  ...
  GetList() {
    this._pressReleaseSummaryReportService
      ...
      .subscribe(
        (data) => {
          if (data.IsSuccess) {
            this.summaryReportList = <DeptCatSubcatModel[]>data.Data.Data;
            this.dataSource = new MatTableDataSource<DeptCatSubcatModel>(
