...
@Component({
  ...
  templateUrl: './date-wise-visitor-count-report.component.html',
  styleUrls: ['./date-wise-visitor-count-report.component.css'],
  providers: [DatePipe,
...
export class DateWiseVisitorCountReportComponent implements OnInit {

  listModel: VisitorCountReportViewModel[];
  dataSource: any;
  totalVCCount: number;
  displayedColumns: string[] = ["index", "CreatedDate", "VisitorCount"];
  ...
  GetList() {
    ...
    this._advertisementService.GetDateWiseVisitorCountReportList(this.indexModel).subscribe(data => {
      if (data.IsSuccess) {
