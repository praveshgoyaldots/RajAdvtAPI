...
import { JankalyanUserLogSummaryReportModel, JankalyanLogSearchModel } from 'src/app/Shared/Model/Master/jankalyanLogMaster.model';
import { MatTableDataSource, MatSort, MatPaginator, MAT_DATE_FORMATS, DateAdapter } from '@angular/material';
...
@Component({
  selector: 'app-jankalyanlog-summary-report',
  templateUrl: './jankalyanlog-summary-report.component.html',
  styleUrls: ['./jankalyanlog-summary-report.component.css'],
  providers: [DatePipe,
    { provide: DateAdapter, useClass: AppDateAdapter },
    { provide: MAT_DATE_FORMATS, useValue: APP_DATE_FORMATS },
  ]
})
export class JankalyanlogSummaryReportComponent implements OnInit {

  listModel: JankalyanUserLogSummaryReportModel[];
  dataSource: any;
  ...
  constructor(
    ...
    this._parentComponent.setpagelayout("Login Log Summary Report :", "", "", "");
  ...
  GetList() {
    ...
    this._jankalyanlogService.GetJankalyanUserLogSummaryReport(this.indexModel).subscribe(
      data => {
        
        if (data.IsSuccess)
         {
          this.listModel = <JankalyanUserLogSummaryReportModel[]>data.Data;
          this.dataSource = new MatTableDataSource<JankalyanUserLogSummaryReportModel>(this.listModel);
  ...
  getTotalCost() {
    ...
    this.totalVCCount= this.listModel.map(t => t.ViewCount).reduce((acc, value) => acc + value, 0);
  }
