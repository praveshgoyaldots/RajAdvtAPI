...
,scmpfc.Name as HowToPayFeeName,scmodc.Name as ModeOfDisbursmentName
,schm.OwnedBy,lOwnedBy.lookup as OwnedByName,lSchm.lookup as ApplyForSchemeName,lmoa.lookup as MadeOfApplingName,schm.OwnedBySate,schm.OwnedByCenter,schm.IsListedRGDPSAct,isrgdps.lookup as IsListedRGDPSActName,schm.DeliveryTimeInDays
...
,schm.StartDate,schm.Logo,schm.Designation,schm.IsLock
...
,(select STUFF( ( SELECT ', ' + CAST(dm.DepartmentTitle AS VARCHAR(100)) FROM [tblSCM_EntryLookUp] el 
	    inner join tblDepartmentMaster as dm on dm.DepartmentCode=el.NodelDepartmentCode
 where el.SchemeID =44 FOR XML PATH('') ), 1, 1, '') AS ProgramAreaName ) as NodelDepartmentName
 ,(case when isnull(schm.ModifiedDate,'')!='' then CONVERT(varchar,schm.ModifiedDate,113)  else CONVERT(varchar,schm.CreatedDate,113)  end) as CraeteDate
