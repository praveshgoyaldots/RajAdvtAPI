...
{
    public class DistrictKPIController : ApiController
    {
		#region /// Variable ///

		private readonly IDistrictKPIService _DistrictKPIService;

		#endregion

		#region /// Constructor ///

		public DistrictKPIController(IDistrictKPIService DistrictKPIService)
		{
			_DistrictKPIService = DistrictKPIService;
		}
		...
		public IHttpActionResult Get(IndexModel model, long catCode = 0)
		{
			...
			try
			{
				return Ok(_DistrictKPIService.GetAll(model, catCode));
			}
			catch (Exception ex)
			{
		...
		public IHttpActionResult Get(long id)
		{
			...
			try
			{
				if (id >= 0)
				{
					return Ok(_DistrictKPIService.GetById(id));
				}
				else
				{
					objReturn.IsSuccess = false;
		...
		/// <returns></returns>      
		[HttpPost]
		public async Task<IHttpActionResult> Post(DistrictKPIModel model)
		{
			...
			try
			{
				...
				{
					//if (!_DistrictKPIService.IsDataAvailable(model.YearCode, model.MonthCode, model.Id))
					//{
					return Ok(await _DistrictKPIService.Create(model));
					//}
					//else
					//{
					//    objReturn.IsSuccess = false;
		...
		/// <returns></returns>
		[HttpPost]
		public async Task<IHttpActionResult> Put(DistrictKPIModel model)
		{
			ServiceResponse<string> objReturn = new ServiceResponse<string>();
			try
			{
				if (ModelState.IsValid)
				{
					//if (!_DistrictKPIService.IsDataAvailable(model.YearCode, model.MonthCode, model.Id))
					//{
					return Ok(await _DistrictKPIService.Edit(model));
					//}
					//else
					//{
					//    objReturn.IsSuccess = false;
		...
		public async Task<IHttpActionResult> UpdateActiveStatus(long id)
		{
			...
			try
			{
				if (id >= 0)
				{
					return Ok(await _DistrictKPIService.UpdateActiveStatus(id));
				}
				else
				{
					objReturn.IsSuccess = false;
		...
		/// <returns></returns>      
		[HttpPost]
		public IHttpActionResult IsDuplicateData(DistrictKPIModel model)
		{
			ServiceResponse<DistrictKPIModel> objReturn = new ServiceResponse<DistrictKPIModel>();
			try
			{
				return Ok(_DistrictKPIService.IsDataAvailable(model));
			}
			catch (Exception ex)
			{
				objReturn.IsSuccess = false;
		...
		public ServiceResponse<List<ComparativeTargetParmeterListModel>> GetAllParameterList(int kPICode, int dpt = 0, long catCode = 0)
		{
			...
			try
			{
				return _DistrictKPIService.GetAllParameterList(kPICode, dpt, catCode);
			}
			catch
			{
				objReturn.Data = null;
