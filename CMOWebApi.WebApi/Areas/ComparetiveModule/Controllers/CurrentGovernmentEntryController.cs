...
namespace CMOWebApi.WebAPI.Areas.ComparetiveModule.Controllers
{
    public class CurrentGovernmentEntryController : ApiController
    {
        ...
        public IHttpActionResult Get(IndexModel model,long catCode=0)
        {
            ...
            try
            {
                return Ok(_CurrentGovernmentEntryService.GetAll(model, catCode));
            }
            catch (Exception ex)
            {
        ...
        public IHttpActionResult GetAllParameterWithCumulativeAndTargetValue(long monthCode = 0, int dpt = 0, int yearCode = 0)
        {
            try
            {
                return Ok(_CurrentGovernmentEntryService.GetAllParameterWithCumulativeAndTargetValue(monthCode, dpt, yearCode));
            }
            catch (Exception ex)
            {
                return InternalServerError(ex);
