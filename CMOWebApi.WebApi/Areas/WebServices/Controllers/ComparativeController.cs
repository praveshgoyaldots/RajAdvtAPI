...
namespace CMOWebApi.WebAPI.Areas.WebServices.Controllers
{
	public class ComparativeController : ApiController
	{
		...
		#region /// constructor //
		public ComparativeController(IDepartmentalProgressService departmentalProgressService)
		{
		...
		/// <returns></returns>
		[HttpPost]
		public async Task<ServiceResponse<string>> DepartmentalPushService(DepartmentalProgressWebServiceModel model)
		{
			...
			try
			{
				if (model != null)
				{
					...
					{
						objReturn = await _departmentalProgressService.DepartmentalPushService(model, Request.Headers);
					}
					else
					{
